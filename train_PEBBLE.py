...
class Workspace(object):
    ...
    def run(self):
        ...
        interact_count = 0
        sr_dz = deque(maxlen=100)
        while self.step < self.cfg.num_train_steps:
            df = pd.DataFrame({'misc/nupdates': [self.step],
                            'ep_rew': episode_reward,
                            'ep_true_rew': true_episode_reward,
                            'ep_success': np.round(np.mean(sr_dz),2)})
            if self.step % 5000 == 0:
                if os.path.exists(os.path.join(self.work_dir, f'{self.cfg.env}.csv')) and self.step > 200:
                    df.to_csv(os.path.join(self.work_dir, f'{self.cfg.env}.csv'), mode='a', header=False, index=False)
                else:
                    df.to_csv(os.path.join(self.work_dir, f'{self.cfg.env}.csv'), mode='w', header=True, index=False)
            if done:
                if self.step > 0:
